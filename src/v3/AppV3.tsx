...
import { D, ISOS, IX2011, IX2018, YEARS, val } from '../lib/metrics.ts';
...
import { FLOWS, countyName, domain, downloadFile, ranked, readState, stateHash, totals, value, type AtlasState, type Explore } from './model.ts';
...
export default function AppV3() {
  ...
  useEffect(() => {
    ...
    const pop = () => { const next = readState(); setLang(next.lang); setS(next); state.current = next; setPlaying(false); setHover(null); };
  ...
  return <div className="v3-app">
    ...
    <header className="v3-header">
      ...
      <div className="v3-header-actions"><VersionSwitch version="v3" />
        ...
        <button className="v3-button v3-share" onClick={() => void share()}><Icon name="share" size={16} /><span>{L('Podijeli', 'Share')}</span></button>
      </div>
    </header>
    <div className="v3-layout">
      <nav className="v3-sidebar" aria-label={L('Glavna navigacija', 'Main navigation')}>
        ...
        <div className="v3-side-bottom"><button ref={aboutTrigger} onClick={showAbout}><Icon name="info" /><span>{L('O atlasu', 'About')}</span></button><span className="v3-side-coordinate">45° N<br />16° E</span></div>
      </nav>
      <main id="v3-explorer" tabIndex={-1}>
        ...
        <section className="v3-workspace" aria-label={L('Istraživanje podataka', 'Explore the data')}>
          <div className="v3-toolbar"><div className="v3-tabs">{(['map', 'trends', 'flows'] as const).map((v, i) => <button key={v} className={s.view === v ? 'is-active' : ''} aria-pressed={s.view === v} onClick={() => selectView(v)}><Icon name={viewIcon(v)} size={17} />{[L('Istraži kartu', 'Explore map'), L('Kroz godine', 'Through the years'), L('Migracijski tokovi', 'Migration flows')][i]}</button>)}</div><button className="v3-export" onClick={exportCSV}><Icon name="download" size={16} />CSV</button></div>
          <div className="v3-filters">{s.view === 'flows' ? <><span className={'v3-data-badge' + (estimated ? ' is-estimate' : '')}><span />{estimated ? L('IPF PROCJENA', 'IPF ESTIMATE') : L('IZMJERENO · 2018.', 'MEASURED · 2018')}</span><div className="v3-segment">{(['in', 'out'] as const).map(d => <button key={d} aria-pressed={direction === d} onClick={() => setDirection(d)}>{d === 'in' ? L('Doseljavanje', 'Arrivals') : L('Odseljavanje', 'Departures')}</button>)}</div><label className="v3-hub-label">{L('Županija', 'County')}<select value={hub} onChange={e => update({ county: e.target.value })}>{ISOS.map(i => <option key={i} value={i}>{countyName(i, s.lang)}</option>)}</select></label></> : <><div className="v3-metrics" aria-label={L('Sastavnica', 'Component')}>{FLOWS.map(f => <button key={f} aria-pressed={s.flow === f} onClick={() => update({ flow: f })}>{metricNames[f]}</button>)}</div><label className="v3-unit"><span className="v3-sr">{L('Jedinica', 'Unit')}</span><select aria-label={L('Jedinica', 'Unit')} value={s.relative ? 'pct' : 'people'} onChange={e => update({ relative: e.target.value === 'pct' })}><option value="people">{L('Broj osoba', 'People')}</option><option value="pct">{L('% popisa 2011.', '% of 2011 census')}</option></select></label></>}</div>
          ...
          </div> : <div className="v3-map-layout"><div className="v3-map-column"><div className="v3-map-heading"><div><h2>{s.view === 'flows' ? countyName(hub, s.lang) : metricFull[s.flow]}</h2><span>{s.view === 'flows' ? direction === 'in' ? L('Odakle ljudi dolaze?', 'Where do people arrive from?') : L('Kamo ljudi odlaze?', 'Where do people move to?') : (s.relative ? L('% stanovništva prema popisu 2011.', '% of 2011 census population') : L('broj osoba', 'number of people'))}<span className="v3-middle-dot">·</span>{period}</span></div><span className="v3-map-meta">{s.view === 'flows' ? '20' : '21'} {s.view === 'flows' ? L('KORIDORA', 'CORRIDORS') : L('ŽUPANIJA', 'COUNTIES')}</span></div>
              <MapCanvas s={s} light={light} hover={hover} onHover={setHover} onSelect={county => update({ county: s.county === county && s.view !== 'flows' ? null : county })} format={format} direction={direction} />
    ...
    <About dialog={dialog} lang={s.lang} onClose={() => aboutTrigger.current?.focus()} />
