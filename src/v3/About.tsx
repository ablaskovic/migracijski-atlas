...
export default function About({ dialog, lang }: { dialog: RefObject<HTMLDialogElement | null>; lang: 'hr' | 'en' }) {
  ...
  return <dialog ref={dialog} className="v3-about" aria-labelledby="v3-about-title"
    ...
    <details><summary>{L('Privatnost i spremljene postavke', 'Privacy and saved preferences')}<Icon name="chevron" size={17} /></summary>
      ...
      <p>{L('Posljednji prikaz svake verzije pamti se u ovoj kartici preglednika. Jezik i tema spremaju se na vaš uređaj. Odabrana županija, godina i ostale postavke analize žive u fragmentu poveznice (#…), koji preglednik ne šalje poslužitelju; fragment se uklanja i iz analitičkih događaja. Nema pretraživanja lokacije putem vanjske usluge.', 'Each version’s last view is remembered in this browser tab. Language and theme are saved on your device. The selected county, year and other analysis settings live in the URL fragment (#…), which the browser does not send to the server; the fragment is also removed from analytics events. No external service is used to look up your location.')}</p>
    </details>
