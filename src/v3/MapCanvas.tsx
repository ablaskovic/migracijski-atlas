...
import { GEO, ISOS, flowMax, flowOf } from '../lib/metrics.ts';
...
export default function MapCanvas({ s, light, hover, onHover, onSelect, format, direction }: Props) {
  ...
  const flowLabel = (iso: string) => iso === hub ? countyName(hub, s.lang) + ' · ' + L('odabrana županija', 'selected county')
    : `${countyName(direction === 'in' ? iso : hub, s.lang)} → ${countyName(direction === 'in' ? hub : iso, s.lang)}: ${count(flowOf(hub, direction, iso, s.yi, s.cum))} ${L('preseljenja', 'moves')}`;
  ...
  return <div className={'v3-cartography' + (zoom > 1 ? ' is-zoomed' : '')}>
    <svg className="v3-map" viewBox={`0 0 ${W} ${H}`} aria-label={L('Interaktivna karta hrvatskih županija', 'Interactive map of Croatian counties')}
      ...
      onPointerMove={e => { const d = drag.current; if (!d) return; const rect = e.currentTarget.getBoundingClientRect(); const factor = Math.max(W / rect.width, H / rect.height); if (Math.hypot(e.clientX - d.x, e.clientY - d.y) > 5) { d.moved = true; e.currentTarget.setPointerCapture(e.pointerId); } if (d.moved) setPan([d.px + (e.clientX - d.x) * factor, d.py + (e.clientY - d.y) * factor]); }}
      ...
      <defs><pattern id="v3-grid" width="45" height="45" patternUnits="userSpaceOnUse"><path d="M 45 0 L 0 0 0 45" fill="none" stroke="currentColor" strokeWidth=".6" /></pattern><marker id="v3-arrowhead" viewBox="0 0 10 10" markerWidth="7" markerHeight="7" refX="9" refY="5" orient="auto" markerUnits="userSpaceOnUse"><path d="M0 0 10 5 0 10Z" fill={direction === 'in' ? 'var(--accent)' : 'var(--coral)'} /></marker></defs>
      ...
      <g transform={`translate(${pan[0]} ${pan[1]}) translate(${W / 2} ${H / 2}) scale(${zoom}) translate(${-W / 2} ${-H / 2})`}>
        <g className="v3-counties">{shapes.map(f => {
          ...
          const description = isFlow ? flowLabel(f.iso) : `${countyName(f.iso, s.lang)}: ${format(n, s.relative)}`;
        ...
        {isFlow && <g className="v3-corridors" aria-hidden="true">{corridors.filter(d => d.n >= 5).map(d => {
          const a = anchors[direction === 'in' ? d.iso : hub], b = anchors[direction === 'in' ? hub : d.iso];
          const bend = Math.hypot(a[0] - b[0], a[1] - b[1]) * .18;
          return <path key={d.iso} d={`M${a[0]},${a[1]} Q${(a[0] + b[0]) / 2 + bend},${(a[1] + b[1]) / 2 - bend} ${b[0]},${b[1]}`}
            fill="none" markerEnd="url(#v3-arrowhead)" stroke={direction === 'in' ? 'var(--accent)' : 'var(--coral)'} strokeWidth={.8 + 6 * Math.sqrt(d.n / maxFlow)} opacity={hover && hover !== d.iso && hover !== hub ? .1 : .65} />;
    ...
    <div className="v3-map-readout" aria-live="polite">{active ? isFlow ? <span>{flowLabel(active)}</span> : <><span>{countyName(active, s.lang)}</span><strong>{format(value(active, s), s.relative)}</strong></> : <><span className="v3-live-dot" /><span>{L('Odaberite županiju za više detalja', 'Select a county to explore')}</span></>}</div>
  </div>;
}
