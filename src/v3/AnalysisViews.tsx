...
export function RegionsView({ s, light, update, format }: Props) {
  ...
  return <div className="v3-analysis" data-analysis="regions"><div className="v3-section-heading"><div><h2>{L('Pet regija. Povezana slika.', 'Five regions. A connected picture.')}</h2><p>{unitName(s)} · {s.cum ? '2011–' : ''}{YEARS[s.yi]}</p></div></div>
    ...
    <div className="v3-legend"><span>{L('Gubitak', 'Loss')}</span><div className="v3-color-key"><div style={{ background: `linear-gradient(90deg,${scale(-RDOM[s.flow + s.den + s.cum])},${scale(0)},${scale(RDOM[s.flow + s.den + s.cum])})` }} /><span>{format(-RDOM[s.flow + s.den + s.cum], s.relative)}</span><span>0</span><span>{format(RDOM[s.flow + s.den + s.cum], s.relative)}</span></div><span>{L('Dobitak', 'Gain')}</span></div>
  </div>;
}
  ...
  return <div className="v3-analysis" data-analysis="matrix"><div className="v3-section-heading"><div><h2>{L('Svaka veza između županija.', 'Every connection between counties.')}</h2><p>{s.dir === 'out' ? L('Redak → stupac: odseljeni', 'Row → column: departures') : s.dir === 'in' ? L('Stupac → redak: doseljeni', 'Column → row: arrivals') : L('Saldo retka: stupac → redak minus redak → stupac', 'Net gain for the row: column → row minus row → column')} · {flowBadge(s.yi, s.cum)}</p></div></div>
    <div className="v3-matrix-scroll" ref={root}><table className="v3-matrix" aria-label={L('Matrica migracija između 21 županije', 'Migration matrix between 21 counties')}><thead><tr><th>{L('Županija', 'County')}</th>{MXORD.map(iso => <th key={iso} scope="col" title={countyName(iso, s.lang)}>{iso.slice(3)}</th>)}</tr></thead><tbody>{MXORD.map((row, ri) => <tr key={row}><th scope="row">{row.slice(3)} · {countyName(row, s.lang)}</th>{MXORD.map((col, ci) => {
      ...
      return <td key={col}>{row === col ? <span className="v3-matrix-diagonal">—</span> : <button data-matrix-cell={idx} title={label} aria-label={label} tabIndex={focus === idx || focus % 22 === 0 && idx === focus + (focus === 440 ? -1 : 1) ? 0 : -1} aria-pressed={s.county === row && s.pair === col} style={{ background: scale(n) }} onClick={() => update({ county: row, pair: col })} onFocus={() => setFocus(idx)} onKeyDown={e => {
        ...
        let next = Math.max(0, Math.min(440, idx + step[e.key]));
        if (next % 22 === 0) next = Math.max(1, Math.min(439, next + (step[e.key] >= 0 ? 1 : -1)));
        root.current?.querySelector<HTMLButtonElement>(`[data-matrix-cell="${next}"]`)?.focus();
      }}><span>{Math.abs(n) >= 1000 ? `${(n / 1000).toFixed(1)}k` : n}</span></button>}</td>;
    })}</tr>)}</tbody></table></div><p className="v3-data-note">{L('Županije su grupirane po regijama. Odaberite ćeliju za oba smjera i godišnju seriju. Tipkovnica: strelice, Home i End.', 'Counties are grouped by region. Select a cell for both directions and the annual series. Keyboard: arrows, Home and End.')}</p>
...
export function PairDetail({ s, update }: { s: AtlasState; update: (patch: Partial<AtlasState>) => void }) {
  ...
  return <section className="v3-pair" data-pair={`${hub}/${partner}`}><div className="v3-panel-title"><h3>{countyName(hub, s.lang)} ↔ {countyName(partner, s.lang)}</h3><button className="v3-icon-button" aria-label={L('Zatvori koridor', 'Close corridor')} onClick={() => update({ pair: null })}><Icon name="close" size={16} /></button></div><p className="v3-data-note">{flowBadge(s.yi, s.cum)} · {s.cum ? '2011–' : ''}{YEARS[s.yi]}</p><div className="v3-pair-stats"><div><span>{L('Prema odabranoj županiji', 'To selected county')}</span><strong>{nf.format(incoming)}</strong></div><div><span>{L('Iz odabrane županije', 'From selected county')}</span><strong>{nf.format(outgoing)}</strong></div><div><span>{L('Saldo odabrane županije', 'Selected county net')}</span><strong className={incoming < outgoing ? 'v3-negative' : ''}>{nf.format(incoming - outgoing)}</strong></div></div><AnnualLines s={s} series={[{ label: `${countyName(partner, s.lang)} → ${countyName(hub, s.lang)}`, color: 'var(--accent)', values: YEARS.map((_, yi) => fsum(partner, hub, yi, false)) }, { label: `${countyName(hub, s.lang)} → ${countyName(partner, s.lang)}`, color: 'var(--coral)', values: YEARS.map((_, yi) => fsum(hub, partner, yi, false)) }]} /><details><summary>{L('Godišnji podaci za oba smjera', 'Annual data for both directions')}</summary><div className="v3-table-scroll"><table className="v3-data-table"><thead><tr><th>{L('Godina', 'Year')}</th><th>→ {countyName(hub, s.lang)}</th><th>→ {countyName(partner, s.lang)}</th><th>{L('Metoda', 'Method')}</th></tr></thead><tbody>{YEARS.map((y, yi) => <tr key={y}><th>{y}</th><td>{nf.format(fsum(partner, hub, yi, false))}</td><td>{nf.format(fsum(hub, partner, yi, false))}</td><td>{flowBadge(yi, false)}</td></tr>)}</tbody></table></div></details></section>;
}
