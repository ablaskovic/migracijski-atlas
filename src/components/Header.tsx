  ...
  return (
    <header className="hd">
      ...
      <div className="ctrls">
        ...
        <div className="ctrl"><span className="ctrl-lab" id="segExpLab">{t('ctrl.export')}</span>
          <div className="seg" id="segExp" role="group" aria-labelledby="segExpLab">
            <button id="pngBtn" disabled={busy} onClick={onPng} title={L('Preuzmi kartu kao PNG', 'Download the map as PNG')}
              aria-label={L('Preuzmi trenutačnu kartu kao PNG', 'Download the current map as PNG')}>{err === 'png' ? L('greška', 'error') : busy ? '…' : 'PNG'}</button>
            <button id="svgBtn" onClick={onSvg} title={L('Preuzmi kartu kao SVG (vektor)', 'Download the map as SVG (vector)')}
