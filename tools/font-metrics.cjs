...
const FACES = [
  ...
  { family: 'Oswald', local: 'Arial Narrow', weights: [500, 600] },
];
...
(async () => {
  ...
  const out = await page.evaluate(async (faces, sample) => {
    ...
    const measure = async (stack, weight) => {
    ...
    for (const f of faces) {
      for (const weight of f.weights) {
        const real = await measure(`'${f.family}'`, weight);
        const plain = await measure(`'${f.local}'`, weight);
        ...
        for (let i = 0; i < 4; i++) {
          ...
          const got = await measure(`'${probe}'`, weight);
        ...
        style.textContent = decl(sizeAdjust);
        const final = await measure(`'${probe}'`, weight);
        document.head.removeChild(style);

        rows.push({
          family: f.family, local: f.local, weight,
  ...
  const worst = Math.max(...out.map(r => Math.abs(r.residual)));
  console.log(`\n/* residual after fitting, on a ${SAMPLE.length}-char sample at a 100 px em:`
